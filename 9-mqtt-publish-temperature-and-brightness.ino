...
public:
    ...
    {
        return currentValue > 0 && currentValue < 1024;
    }
};
...
int groupId = 99; // id of your group, should be 1-5
...
String clientName = String("swec") + String(groupId);

// recieve message
void mqtt_callback(char* topic, byte* payload, unsigned int length)
{
    // nothing to do, as we publish only
}
...
void loop()
{
    ...
    if (mqtt.isConnected())
    {
        if (temp.hasValidTemperature())
            mqtt.publish("swec/temperature", String(groupId) + String("/") + String(temp.getCelsius()));
    
        if (photo.hasValidBrightness())
            mqtt.publish("swec/brightness", String(groupId) + String("/") + String(photo.getValue()));
    }
